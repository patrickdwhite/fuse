...
struct fuse_operations {
	...
	void readlink (in const(char)[] path, ubyte[] buf);

	void mknod (in const(char)[] path, mode_t mode, dev_t dev);

	void mkdir (in const(char)[] path, mode_t mode);

	void unlink (in const(char)[] path);

	void rmdir (in const(char)[] path);

	void symlink (in const(char)[] to, in const(char)[] path);

	void rename (in const(char)[] path, in const(char)[] to);

	void link (in const(char)[] path, in const(char)[] to);

	void chmod (in const(char)[] path, mode_t mode);

	void chown (in const(char)[] path, uid_t uid, gid_t gid);

	void truncate (in const(char)[] path, off_t length);

	void open (in const(char)[] path, fuse_file_info *info);

	int read (in const(char)[] path, ubyte[] readbuf, off_t offset,
			fuse_file_info * info );

	int write (in const(char)[] path, in const(ubyte)[] data, off_t offset, fuse_file_info *info);

	void statfs (in const(char)[] path, statvfs_t *stbuf);

	void flush (in const(char)[] path, fuse_file_info *info);

	void release (in const(char)[] path, fuse_file_info *info);

	void fsync (in const(char)[] path, bool onlyldatasync, fuse_file_info *info);

	void setxattr (in const(char)[] path, in const(char)[] name, in const(ubyte)[] data, int flags);

	ssize_t getxattr (in const(char)[] path, in const(char)[] name, ubyte[] data);

	ssize_t listxattr (in const(char)[] path, char[] list);

	void removexattr (in const(char)[] path, in const(char)[] name);

	void opendir (in const(char)[] path, fuse_file_info *info);

	void readdir (in const(char)[] path, void * buf, fuse_fill_dir_t filler, off_t offset,
			fuse_file_info *info);

	void releasedir (in const(char)[] path, fuse_file_info *info);

	void fsyncdir (in const(char)[] path, int, fuse_file_info *info);
	
	void* init (fuse_conn_info *conn);

	void destroy (void *private_data);

	void access (in const(char)[] path, int mask);

	void create (in const(char)[] path, mode_t mode, fuse_file_info *info);

	void ftruncate (in const(char)[] path, off_t length, fuse_file_info *info);

	void fgetattr (in const(char)[] path, stat_t *stbuf, fuse_file_info *info);

	void lock (in const(char)[] path, fuse_file_info *info, int cmd, flock *locks);

	void utimens (in const(char)[] path, const timespec tv[]);

	void bmap (in const(char)[] path, size_t blocksize, ulong *idx);
	


	int ioctl (in const(char)[] path, int cmd, void *arg, fuse_file_info *, uint flags, void *data);

	int poll (in const(char)[] path, fuse_file_info *info,	fuse_pollhandle *ph, uint *reventsp);
